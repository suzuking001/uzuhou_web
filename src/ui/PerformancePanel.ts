...
export class PerformancePanel {
  ...
  update(snapshot: PerformanceSnapshot, simulationTime: number): void {
    ...
    this.set('metric-algorithm', snapshot.algorithmMode === 'direct' ? 'Direct' : `Hybrid ${snapshot.gridResolution}²`);
    ...
    this.set('complexity-footer', `EDUCATIONAL CFD · ${snapshot.algorithmMode === 'direct' ? 'DIRECT O(N²)' : `HYBRID GRID ${snapshot.gridResolution}²`} · f32\n認証解析・安全判断には使用しないでください`);
  }

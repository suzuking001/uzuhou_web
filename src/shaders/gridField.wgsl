...
@group(0) @binding(4) var<storage, read_write> field: array<vec4<f32>>;
...
fn velocityAt(position: vec2<f32>, resolution: u32) -> vec2<f32> {
  ...
  for (var dy: i32 = -1; dy <= 1; dy += 1) {
    ...
    for (var dx: i32 = -1; dx <= 1; dx += 1) {
      ...
      for (var member = 0u; member < count; member++) {
        velocity += directVelocity(particles[memberIndices[cellIndex * MAX_PARTICLES + member]], position);
      }
    }
  }
  return velocity;
...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
  ...
  var velocity = velocityAt(position, max(sim.displayFlags, 1u));
